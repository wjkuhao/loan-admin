...
/**
 * <p>
 * Title: <font color=red>格式化文本</font>
 * </p>
 * <p>
 * Description:
 * </p>
...
public class HtmlUtil {
	 ...
	 * 
	 * @param source
	 *            <font color=red>原始字符</font>
	 * @param div
	 *            <font color=red>分割符</font>
	 ...
	 * 
	 * @param str
	 *            <font color=red>原始字符串</font>
	 * @param substr
	 *            <font color=red>要替换的字符</font>
	 * @param restr
	 *            <font color=red>替换后的字符</font>
	 ...
	 * 
	 * @param i
	 *            <font color=red>三位随机数的最后一位</font>
	 * @return <font color=red>返回一个字符串</font>
	 */
...
//			ex.printStackTrace();
	...
	public static String filterXmlChar(String s)
    {
	...
	public static String clearJavaScriptAgain(String text) {
		if (text != null) {
			text = text.replaceAll("var(.|\\s)*\\}", "");
			text = text.replaceAll("function(.|\\s)*\\}", "");
		}
		return text;
	}

	public static String clearBrOrP(String text) {
		if (text != null) {
			text = text.replaceAll("\\n\\ *(<\\/br>|<br>)*\\ *\\n", "\n");// 使用“\n”替换所有无用的<br>
			text = text.replaceAll("<p>\\s*<\\/p>", "");// 使用空字符串替换所有无用<p></p>标签
		}
		return text;
	}

	public static String picHtm(String text) {
		String[] arrybody = text.split("<img");
		String str = "";
		for (int i = 0; i < arrybody.length; i++) {
			String a = arrybody[i];
			if (i == 0) {
				a = "<p>" + a;
			} else {
				if (a.contains("src")) {
					if (a.contains("<p>")) {
						a = "<p><center><img"
								+ a.substring(0, a.indexOf("/>") + 2)
								+ "</center><p><center>"
								+ a.substring(a.indexOf("/>") + 2,
										a.indexOf("<p>")) + "</center></p></p>"
								+ a.substring(a.indexOf("<p>"));
					} else {
						a = "<p><center><img"
								+ a.substring(0, a.indexOf("/>") + 2)
								+ "</center><p><center>"
	...
	public static String changeVideoCode(String s, String videoPath, String imagePath, String idStr) {
		if (s.indexOf("<video") != -1) {
			int aa = s.indexOf("<video");
			String preStr = s.substring(0, aa + 12);
			int bb = s.indexOf(".mp4");
			if(bb == -1) {
			 bb = s.indexOf(".3gp");
			}
			String endStr = s.substring(bb + 6);
			System.out.println(preStr + videoPath + "\"" + " poster=" + "\""
					+ imagePath + "\"" + " id=" + "\"" + idStr + "\"" + " "+ endStr);
			return preStr + videoPath + "\"" + " poster=" + "\""
			+ imagePath + "\"" + " id=" + "\"" + idStr + "\"" + " "+ endStr;
	...
	public static String htmlEncodeForVmag(String txt) {
		if (txt != null) {
			txt = replace(txt.trim(), "&", "&amp;");
			//txt = replace(txt.trim(), "\"", "&quot;");
			//txt = replace(txt.trim(), "<", "&lt;");
			//txt = replace(txt.trim(), ">", "&gt;");
			txt = replace(txt.trim(), "\'", "&acute;");
			if(txt.indexOf("<a href=") != -1) {
				int b= txt.indexOf("<a href=");
				int j= txt.indexOf(">");
				int h = txt.indexOf("</a>");
				String fff = txt.substring(b, j+1);
				String www = txt.substring(b +9, j-1);
				String kkk = txt.substring(j +1, h);
				String ttt = txt.replace(fff, "<URL>");
				String mmm = ttt.replace("</a>", "</URL>");
				String nnn = mmm.replace(kkk, kkk + ";" + www);
...
//				System.out.println(fff);
	...
	public static String unHtmlEncodeForVmag(String txt) {
		if (txt != null) {
			txt = replace(txt.trim(),"&amp;", "&");
			txt = replace(txt.trim(),"&quot;", "\"");
			txt = replace(txt.trim(),"&lt;", "<" );
	...
	public static void main(String[] argv) {
		String a = "<script>sss</script>吴光宇";
		String clearJavaScriptAgain = HtmlUtil.clearJavaScriptAgain(a);
		System.out.print(HtmlUtil.htmlcode(a));
